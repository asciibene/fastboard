...
class Database
{
  ...
  {
    return ['users' => $this->users, 'threads' => $this->threads];
  }
  function __unserialize($sdata){
    $this->users = $sdata['users'];
    $this->threads = $sdata['threads'];
  }
...
class User
{
  ...
  function cast_vote()
  {
    global $db;
    if (
      isset($_POST['tuid']) and
      key_exists($_POST['tuid'], $db->threads) and
      is_integer($_POST['voteval'] ?? false)
    ) {
